...
const AssetModal = ({ showAssets, handleCloseAssets, handleAdd }) => {
  ...
  useEffect(() => {
    axios.get(`${baseURL}pms/inventory_types.json?q[category_eq]=asset&token=${token}`)
      .then(response => {
  ...
  useEffect(() => {
    if (selectedInventory) {
      ...
      axios.get(`${baseURL}pms/inventories.json?q[inventory_type_id_in]=${inventoryTypeIds}&q[material_category_eq]=asset&token=${token}`)
        .then(response => {
          // Map the sub-types to options for the select dropdown
          const options = response.data.map(subType => ({
            value: subType.id,
  ...
  const handleGoButtonClick = () => {
    ...
    const apiUrl = `${baseURL}pms/inventories.json?q[inventory_type_id_in]=${inventoryTypeIds}&q[id_in]=${inventoryMaterialTypeIds}&q[material_category_eq]=asset&token=${token}`;
