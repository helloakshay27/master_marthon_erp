...
const MaterialModal = ({ show, handleClose, handleAdd }) => {
  ...
  useEffect(() => {
    axios.get('https://newerp.marathonrealty.com/pms/inventory_types.json?q[category_eq]=material&token=bfa5004e7b0175622be8f7e69b37d01290b737f82e078414')
      .then(response => {
  ...
  useEffect(() => {
    if (selectedInventory) {
      ...
      axios.get(`https://newerp.marathonrealty.com/pms/inventories.json?q[inventory_type_id_in]=${inventoryTypeIds}&q[material_category_eq]=material&token=bfa5004e7b0175622be8f7e69b37d01290b737f82e078414`)
        .then(response => {
          // Map the sub-types to options for the select dropdown
          const options = response.data.map(subType => ({
            value: subType.id,
  ...
  const handleGoButtonClick = () => {
    ...
    const apiUrl = `https://newerp.marathonrealty.com/pms/inventories.json?q[inventory_type_id_in]=${inventoryTypeIds}&q[id_in]=${inventoryMaterialTypeIds}&q[material_category_eq]=material&token=bfa5004e7b0175622be8f7e69b37d01290b737f82e078414`;
