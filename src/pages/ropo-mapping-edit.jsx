...
const RopoMappingEdit = () => {
  ...
  // Fetch ROPO mapping details and prefill form/table
  useEffect(() => {
    const fetchMapping = async () => {
      if (!id || !token) return;
      try {
        const resp = await axios.get(`${baseURL}ropo_mappings/${id}.json?token=${token}`);
        const data = resp.data || {};
        setMappingDetails(data);

        // Prefill date and remarks
        setMappingDate(data.mapping_date || "");
        setRemarks(data.remarks || "");

        // Prefill project and site (fallback to API values if projects list not loaded yet)
        // const projectOption = data.project
        //   ? {
        //       value: data.project.id,
        //       label: data.project.name,
        //       pms_sites: data.pms_site
        //         ? [
        //             {
        //               value: data.pms_site.id,
        //               label: data.pms_site.name,
        //             },
        //           ]
        //         : [],
        //     }
        ...
        //   setProjects((prev) => {
        //     if (prev.some((p) => p.value === projectOption.value)) return prev;
        //     return [...prev, projectOption];
        //   });
        //   setSiteOptions(projectOption.pms_sites || []);
        // }
        // if (data.pms_site) {
        //   setSelectedSite({ value: data.pms_site.id, label: data.pms_site.name });
        // }
        // After the project and site handling code, add:
const companyOption = data.company 
  ? {
...
if (companyOption) {
  ...
  setCompanyOptions((prev) => {
    if (prev.some((c) => c.value === companyOption.value)) return prev;
    return [...prev, companyOption];
  });
}

        // Build MOR grouping for table (dedupe materials by mor_inventory_id)
        const morGroups = {};
        (data.ropo_mor_inventories || []).forEach((item) => {
          const key = item.mor_number;
          if (!morGroups[key]) {
            morGroups[key] = {
              mor_id: key, // use MOR number as stable id for grouping
              mor_number: key,
              project_name: data.project?.name || "",
              sub_project_name: data.pms_site?.name || "",
              mor_inventories: [],
            };
          }
          const exists = morGroups[key].mor_inventories.some(
            (inv) => inv.id === item.mor_inventory_id
          );
          if (!exists) {
            morGroups[key].mor_inventories.push({
        ...
        setMorData(Object.values(morGroups));

        // Set PO data so PO rows show under each material
        const poRows = (data.ropo_mor_inventories || []).map((item) => ({
          ropo_mor_inventory_id: item.id,
          mor_inventory_id: item.mor_inventory_id,
          po_mor_inventory_id: item.po_mor_inventory_id,
          po_number: item.po_number,
          uom_name: item.uom_name,
          rate_per_nos: item.rate_per_nos,
          required_quantity: item.required_quantity,
          prev_order_qty: item.prev_order_qty,
          pending_qty: item.pending_qty,
          order_qty: item.order_qty,
        ...
        setPoData(poRows);
        setInitialRopoMorInventoryIds(poRows.map(r => r.ropo_mor_inventory_id).filter(Boolean));

        // Initialize ordered quantities for PO rows using order_qty from API
        const initialOrdered = {};
        poRows.forEach((row) => {
          const key = `${row.purchase_order_id || row.po_mor_inventory_id}-${row.mor_inventory_id}`;
          if (row.order_qty !== undefined && row.order_qty !== null) {
            initialOrdered[key] = row.order_qty;
          }
        });
        setOrderedQuantities(initialOrdered);
      } catch (e) {
        console.error("Error fetching ROPO mapping:", e);
      }
    };
  ...
  const [aggregatedPendingByMaterial, setAggregatedPendingByMaterial] = useState({});
  const [initialRopoMorInventoryIds, setInitialRopoMorInventoryIds] = useState([]);
  ...
  const fetchMaterialDetails = async (useFilters = true, overrides = null) => {
    ...
    try {
      ...
      if (useFilters) {
        // Include company filter from main form
        if (selectedCompany?.value) {
          queryParams.append("q[company_id_in][]", selectedCompany.value);
        }
        // Include multi project and site filters from modal selections
        const projectList = overrides?.projectIds ?? (Array.isArray(selectedProject) ? selectedProject.map(o => o.value) : []);
        const siteList = overrides?.siteIds ?? (Array.isArray(selectedSite) ? selectedSite.map(o => o.value) : []);
        if (Array.isArray(projectList) && projectList.length > 0) {
          projectList.forEach((id) => queryParams.append("q[project_id_in][]", id));
        }
        if (Array.isArray(siteList) && siteList.length > 0) {
          siteList.forEach((id) => queryParams.append("q[pms_site_id_in][]", id));
        }
  ...
  const handleAcceptSelectedMaterials = () => {
    ...
    // Prevent adding materials whose MOR inventory IDs already exist in the table
    const existingInventoryIds = new Set(
      morData.flatMap((mor) => mor.mor_inventories.map((inv) => inv.id))
    );
    const selectedInventoryIds = selectedMaterials.flatMap((m) =>
      m.mor_inventories.map((inv) => inv.id)
    );
    const duplicateInventoryIds = selectedInventoryIds.filter((id) =>
  ...
  const handleMorReset = () => {
    ...
    // Clear project/sub-project selections like create page
    ...
    setMaterialDetailsData([]);
    setSelectedMaterialItems([]);
    // Fetch with company-only filter immediately after reset
    if (selectedCompany?.value) {
      fetchMaterialDetails(true, { projectIds: [], siteIds: [] });
    }
  };
...
const [remainingQuantities, setRemainingQuantities] = useState({});
  ...
  // Aggregate per-material pending qty from PO rows (take only valid numeric values)
  useEffect(() => {
    const sumByMaterial = {};
    const validFlagByMaterial = {};
    (poData || []).forEach((row) => {
      const materialId = row.mor_inventory_id;
      const raw = row?.pending_qty;
      const val = raw !== null && raw !== undefined && !Number.isNaN(parseFloat(raw))
        ? parseFloat(raw)
        : null;
      if (val !== null) {
        sumByMaterial[materialId] = (sumByMaterial[materialId] || 0) + val;
        validFlagByMaterial[materialId] = true;
      }
    });
    const pendingMap = {};
    Object.keys(validFlagByMaterial).forEach((mid) => {
      pendingMap[mid] = sumByMaterial[mid];
    });
  ...
  const getBasePendingForMaterial = (materialId) => {
    ...
    return material.pending_qty !== null && material.pending_qty !== undefined
      ? parseFloat(material.pending_qty)
      : (parseFloat(material.required_quantity || 0) - parseFloat(material.prev_order_qty || 0));
  };

  // Helper: compute available qty for a given (poId, materialId)
  const computeAvailableQty = (poId, materialId) => {
    ...
    const available = basePendingQty - othersOrderedQty;
  ...
  const handleRopoMappingSubmit = async () => {
    ...
    try {
      ...
      // Build PATCH payload with ids and _destroy flags
      const toDestroyIds = initialRopoMorInventoryIds.filter((rid) => {
        // if initial id is not present in current poData, mark for destroy
        return !poData.some((row) => row.ropo_mor_inventory_id === rid);
      });
      const toDestroyAttributes = toDestroyIds.map((id) => ({ id, _destroy: true }));

      const toCreateAttributes = [];
      const toUpdateAttributes = [];
      // From computed attributes, if record exists (has ropo_mor_inventory_id), include id for update; otherwise, create
      ropoMorInventoriesAttributes.forEach((attr) => {
        const matched = poData.find(
          (row) => row.mor_inventory_id === attr.mor_inventory_id && (row.po_mor_inventory_id === attr.po_mor_inventory_id || !row.po_mor_inventory_id)
        );
        if (matched && matched.ropo_mor_inventory_id) {
          toUpdateAttributes.push({ id: matched.ropo_mor_inventory_id, ...attr });
        } else {
          toCreateAttributes.push(attr);
        }
      });
      ...
      const patchPayload = {
        ropo_mapping: {
          ...
          remarks: remarks || "Some notes",
          status: status,
          ropo_mor_inventories_attributes: [
            ...toUpdateAttributes,
            ...toCreateAttributes,
            ...toDestroyAttributes,
          ],
      ...
      const response = await axios.patch(
        `${baseURL}ropo_mappings/${id}.json?token=${token}`,
        patchPayload,
        { headers: { "Content-Type": "application/json" } }
      );
...
setPoModalApiData(
  ...
  useEffect(() => {
    if (addMORModal) {
      setSelectedMaterialItems([]);
      // Prefetch using company-only filter (no project/site)
      if (selectedCompany?.value) {
        fetchMaterialDetails(true, { projectIds: [], siteIds: [] });
      }
    }
  ...
  // Do not auto-fetch on project/sub-project change; Search triggers fetching
    ...
    return base.toFixed(2);
                                                  ...
                                                  {collapsedMORs[
                                                    ...
                                                    poData
                                                      ...
                                                      .map((poItem, index) => (
                                                        <tr
                                                          key={`po-${poItem.purchase_order_id || poItem.po_mor_inventory_id}-${inventory.id}`}
...
<td>
  <input
    ...
    value={orderedQuantities[`${poItem.purchase_order_id || poItem.po_mor_inventory_id}-${poItem.mor_inventory_id}`] ?? poItem.order_qty ?? ""}
    onChange={(e) => handleOrderedQuantityChange(
      poItem.purchase_order_id || poItem.po_mor_inventory_id,
      poItem.mor_inventory_id,
      e.target.value
    )}
    min="0"
    max={remainingQuantities[poItem.mor_inventory_id] || 0}
                ...
                <div className="col-md-4">
                  <div className="form-group">
                    ...
                    <MultiSelector
                      ...
                      isDisabled={!selectedProject || (Array.isArray(selectedProject) && selectedProject.length === 0)}
                    />
                  </div>
                </div>
...
export default RopoMappingEdit;
