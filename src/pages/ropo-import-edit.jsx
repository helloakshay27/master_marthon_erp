...
const RopoImportEdit = () => {
  ...
  const fetchEditData = async () => {
    try {
      const response = await axios.get(
        `${baseURL}purchase_orders/${poId}/ropo_detail.json?token=${token}&po_type=import`
      );
  ...
  const populateFormData = (data) => {
    ...
    const selectedCompany = companies.find(company => company.id === data.company_id);
    const companyOption = selectedCompany ? { value: selectedCompany.id, label: selectedCompany.name } : null;

    // Find the matching supplier from the loaded suppliers list
    const selectedSupplier = suppliers.find(supplier => supplier.id === data.supplier_id);
    const supplierOption = selectedSupplier ? { value: selectedSupplier.id, label: selectedSupplier.name } : null;

    setFormData(prev => ({
      ...prev,
      company: companyOption,
      supplier: supplierOption,
      currency: data.po_currency ? { value: data.po_currency.toLowerCase(), label: data.po_currency.toUpperCase() } : null,
      ...
      payableToServiceProvider: data.payable_to_service_provider?.toString() || "",
  ...
  const fetchProjectsAndSites = async (data) => {
    try {
      // Fetch projects
      const projectsResponse = await axios.get(`${baseURL}projects.json?token=${token}`);
      setProjects(projectsResponse.data.projects || []);

      // Fetch sites
      const sitesResponse = await axios.get(`${baseURL}sites.json?token=${token}`);
      ...
      if (data.material_details && data.material_details.length > 0) {
        ...
        if (firstMaterial.project_id) {
          const selectedProject = projectsResponse.data.projects?.find(p => p.id === firstMaterial.project_id);
          if (selectedProject) {
            setFormData(prev => ({
              ...prev,
              project: { value: selectedProject.id, label: selectedProject.name }
            }));
          }
        }
        if (firstMaterial.sub_project_id) {
          const selectedSite = sitesResponse.data.sites?.find(s => s.id === firstMaterial.sub_project_id);
          if (selectedSite) {
            setFormData(prev => ({
              ...prev,
  ...
  const populateMaterialDetails = (materialDetails) => {
    const populatedMaterials = materialDetails.map(material => ({
      ...
      project: material.project_id ? { value: material.project_id, label: material.project } : null,
      subProject: material.sub_project_id ? { value: material.sub_project_id, label: material.sub_project } : null,
  ...
  const populateOtherSections = (data) => {
    ...
    // Populate charges data from mor_inventory_tax_details
    if (data.mor_inventory_tax_details) {
      setChargesFromApi(data.mor_inventory_tax_details);
    }
    ...
    if (data.terms_and_conditions) {
      ...
      setTermsFormData(prev => ({
        ...
        advanceReminderDuration: terms.advance_reminder_duration?.toString() || "",
    ...
    if (data.resource_term_conditions) {
      const populatedTermConditions = data.resource_term_conditions.map(term => ({
        id: term.id,
      ...
      const formattedGeneralTerms = data.resource_term_conditions.map(term => ({
        id: term.id,
  ...
  const getConsolidatedCharges = useCallback(() => {
    ...
    const groupedCharges = taxCharges.reduce((acc, charge) => {
      const chargeName = charge.resource_name || "Unknown";
      ...
      acc[chargeName].total_amount_inr += parseFloat(charge.amount_in_inr || 0);
                ...
                if (rateData.addition_tax_details && Array.isArray(rateData.addition_tax_details)) {
                  rateData.addition_tax_details.forEach((tax) => {
                    updatedChargesData.push({
                      ...
                      material_name: material.material_name || material.material?.material_name || "",
  ...
  const handleOpenTaxesModal = (itemId, itemType = "charge") => {
    ...
    if (item?.taxes_and_charges && item.taxes_and_charges.length > 0) {
      ...
      savedTaxes = {
        ...
        netCost: (baseCost + additionTaxes.reduce((sum, tax) => sum + parseFloat(tax.amount || 0), 0) - deductionTaxes.reduce((sum, tax) => sum + parseFloat(tax.amount || 0), 0)).toFixed(2)
      };
    } else {
                                    ...
                                    {submittedMaterials.length > 0 ? (
                                      submittedMaterials.map((row, index) => (
                                        <tr key={index}>
                                          ...
                                          <td className="text-start">
                                            {row.material_name || row.material?.material_name || ""}
                                          </td>
                                          <td className="text-start">
                                            {row.uom_name || ""}
                                          </td>
                              ...
                              <table className="w-100">
                                ...
                                <tbody>
                                  <tr>
                                    ...
                                    <td>{taxSummary?.total_base_cost_in_inr || "0"}</td>
                                    <td>{taxSummary?.total_base_cost || "0"}</td>
                                  </tr>
                                  <tr>
                                    <td>Total Tax</td>
                                    <td>{taxSummary?.total_tax_in_inr || "0"}</td>
                                    <td>{taxSummary?.total_tax || "0"}</td>
                                  </tr>
                                  <tr>
                                    <td>Total Charge</td>
                                    <td>{taxSummary?.total_charge_in_inr || "0"}</td>
                                  ...
                                  <tr>
                                    ...
                                    <td className="fw-bold">{taxSummary?.total_inclusive_cost_in_inr || "0"}</td>
                    ...
                    {materialDetailsData.length > 0 ? (
                      materialDetailsData.map((item, index) => (
                        ...
                        >
                          ...
                          <td>{item.material_name || item.material?.material_name || ""}</td>
                          <td>{item.uom_name || ""}</td>
