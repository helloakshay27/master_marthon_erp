    ...
    // --- Labour Activities Create Handler ---
        ...
        // };
         ...
         const handleCreateLabour = (e) => {
                ...
                // Duplicate check: prevent same activity or sub-activity in tableData (already added rows, including API-fetched)
                ...
                labourRows.forEach((row, rowIndex) => {
                    if (row.subActivities && row.subActivities.length > 0) {
                        row.subActivities.forEach(sub => {
                            // Check for duplicate activity+subActivity in tableData (API + new), include resource_id for API-fetched rows
                            const exists = tableData.some(td => {
                                // If API row has resource_id, use it for comparison, else use main/sub categories
                                if (td.resource_id) {
                                    // Compare resource_id and activity/subActivity
                                    return (
                                        td.activity === row.activity &&
                                        td.subActivity === sub.subActivity &&
                                        td.resource_id === (selectedSubCategoryLevel5?.value || selectedSubCategoryLevel4?.value || selectedSubCategoryLevel3?.value || selectedSubCategory?.value || selectedCategory?.value)
                                    );
                                } else {
                                    return (
                                        td.activity === row.activity &&
                        ...
                        // Check for duplicate activity (no sub-activity) in tableData (API + new), include resource_id for API-fetched rows
                        const exists = tableData.some(td => {
                            if (td.resource_id) {
                                return (
                                    td.activity === row.activity &&
                ...
                if (duplicateInTable) {
                    toast.error("This Labour Activity already exists in the table (including already saved data).");
                    return;
                }
                if (duplicateSubInTable) {
                    toast.error("This Sub-Activity for the selected Activity already exists in the table (including already saved data).");
                    return;
                }
                ...
                <div className="module-data-section p-4">
                                            ...
                                            tableData.map((row, index) => {
                                                ...
                                                return (
                                                    <tr key={index}>
                                                        ...
                                                        <td className="text-start" style={{ width: "170px" }}>
                                                            <input
                                                                type="date"
                                                        ...
                                                        <td className="text-start" style={{ maxWidth: "170px" }}>
                    ...
                    <div className="row mt-2 justify-content-center mb-5 pb-5">
                        ...
                        <div className="col-md-2">
                            <button className="purple-btn1 w-100" onClick={() => navigate(`/labour-rate-list?token=${token}`)}> Cancel</button>
                        </div>
                    </div>
                </div>
            </div>
