...
import ChecklistTableCard from "../components/common/Card/ChecklistTableCard";
...
export default function VendorDetailsService() {
  ...
  const handleSubmit = async () => {
    ...
    console.log("Final payload with checklist:", payload);
    const urlParams = new URLSearchParams(location.search);
    const token = urlParams.get("token");
    try {
      const payload = preparePayload();
  ...
  const confirmReviseBid = async () => {
    ...
    try {
      const firstBidId = Array.isArray(bidIds) ? bidIds[0] : bidIds;
      ...
      const response = await axios.post(
        `${baseURL}/rfq/events/${eventId}/bids/${firstBidId}/revised_bids?token=${token}&event_vendor_id=${vendorId}`,
        payload
      );
  ...
  const [checklistPayload, setChecklistPayload] = useState({});
  ...
  const handleChecklistDataChange = (checklistData) => {
    setChecklistPayload(checklistData);
  };
                      ...
                      <div className="w-100  pt-2 mb-2  pe-2">
                        ...
                        <div className="card mb-5 p-3 mt-2 rounded-3 ">
                          ...
                          >
                            ...
                            {documentAttachment && (
                              <div id="document-attachment" className="mt-2" style={{ paddingLeft: "24px" }}>
                                <div className="card card-body rounded-3 p-4">
                                  ...
                                  {data1.attachments?.length > 0 ? (
                                    <div className="tbl-container mt-3">
                                      <table className="w-100 table">
                                        ...
                                        <tbody>
                                          {data1.attachments?.map((attachment, index) => {
                                    ...
                                    <p className="text-center mt-4">No attachments available for this event.</p>
                                  )}
                                </div>
                              </div>
                            )}
              ...
              >
                ...
                <div style={{ marginTop: "10px" }}>
                  ...
                  <div style={{ marginTop: "30px" }}>
                    ...
                    <p className="head-material text-muted">
                      Please find below the terms and conditions associatedm
                      with the orders
                    </p>
                    <ol
                      className="head-material"
                  ...
                  {data1?.checklist_details && data1.checklist_details.length > 0 && (
                    <div className="card p-0 mt-5">
                      <div className="card-header3">
                        <h3 className="card-title" style={{ fontWeight: 500, fontSize: "16px" }}>
                          Technical Pre-Qualification
                        </h3>
                      </div>
                      <div className="card-body p-0">
                        {data1?.checklist_details && data1.checklist_details.length > 0 && (
                          <div id="checklist-section" className="mt-0 mx-3">
                            <ChecklistTableCard 
                              checklistData={data1.checklist_details} 
