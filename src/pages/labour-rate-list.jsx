...
const LabourRateList = () => {
        ...
        reader.onload = async (event) => {
            ...
            try {
                const response = await axios.post(
                    `${baseURL}labour_rate_details/import.json?token=${token}`,
                    { file: base64String },
    ...
    useEffect(() => {
        ...
        axios
            .get(
                `${baseURL}labour_rate_details.json?q[id_eq]=&q[projects_id_eq]=&q[pms_sites_id_eq]=&q[pms_wings_id_eq]=&token=${token}`
            )
    ...
    const handleClick = () => {
        ...
        // setShowModal(true);
        navigate(`/labour-rate-create?token=${token}`); // This will navigate to /labour-rate-create
    };
    ...
    const fetchFilteredData = async () => {
        ...
        try {
            const response = await axios.get(
                `${baseURL}labour_rate_details.json?q[id_eq]=${companyId}&q[projects_id_eq]=${projectId}&q[pms_sites_id_eq]=${siteId}&q[pms_wings_id_eq]=${wingId}&token=${token}`
            );
            setData(response.data);
        } catch (error) {
        ...
        <>
            <div className="website-content overflow-auto">
                <div className="module-data-section p-4 pb-5">
                    ...
                    <h5 class="mt-4">Rate Card (Labour)</h5>
                                                            ...
                                                            <tr key={`${companyIndex}-${projectIndex}-${siteIndex}-${wingIndex}`}>
                                                                ...
                                                                {wingIndex === 0 && siteIndex === 0 ? (
                                                                    <td className="text-start">
                                                                        {project.rate_id ? (
                                                                            <a href={`/labour-rate-details/${project.rate_id}?token=${token}`}
                                                                ...
                                                                {wingIndex === 0 ? (
                                                                    <td className="text-start">
                                                                        {site.rate_id ? (
                                                                            <a href={`/labour-rate-details/${site.rate_id}?token=${token}`}
                                                                ...
                                                                <td className="text-start">
                                                                    {/* {wing.name} */}
                                                                    {wing.rate_id ? (
                                                                        <a href={`/labour-rate-details/${wing.rate_id}?token=${token}`}
