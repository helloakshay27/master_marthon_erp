...
const PodDomesticDetail = () => {
  ...
  // Resolve Tax / Charge Type label for display (handles TaxCategory and TaxCharge)

  const getTaxChargeLabel = useCallback(
    (item) => {
      if (!item) return "";
      if (item.taxChargeType) return item.taxChargeType;

      const baseId = item.tax_category_id || item.resource_id;
      if (!baseId) return "";

      const fromAddOptions = taxOptions.find((opt) => opt.id === baseId)?.value;
      if (fromAddOptions) return fromAddOptions;

      const fromDeductById = deductionTaxOptions.find(
        (opt) => opt.id === baseId
      );
      if (fromDeductById?.label) return fromDeductById.label;
      if (fromDeductById?.value) return fromDeductById.value;
    ...
    const fetchRopoData = async () => {
      try {
        ...
        const response = await axios.get(
          `${baseURL}purchase_orders/${256}/ropo_detail.json?token=${token}&po_type=import`
        );
                              ...
                              <div className="d-flex gap-1 justify-content-end mb-4">
                                ...
                                {ropoData?.selected_status?.toLowerCase() !==
                                  ...
                                  ropoData?.vendor_status?.toLowerCase() !==
                                    "accepted" && (
                                    <Link
                                      to={`/ropo-import-ammend/${ropoData?.parent_po_id}?token=${token}`}
                                      className="d-flex align-items-center"
                                      style={{ borderColor: "#8b0203" }}
                                    >
                                      <button
...
{/* 
                                    ...
                                    <td>
                                      {`INR ${formatTwoDecimals(item.material_rate_in_inr)}`}
                                    </td>

                                    <td>
                                      {`INR ${formatTwoDecimals(item.material_cost_in_inr)}`}
                                    </td>

                                    <td>{item?.discount_percentage != null ? formatTwoDecimals(item.discount_percentage) : "-"}</td>

                                    <td>
                                      {`INR ${formatTwoDecimals(item.discount_rate_in_inr)}`}
                                    </td>

                                    <td>
                                      {`INR ${formatTwoDecimals(item.after_discount_value_in_inr)}`}
                                    </td>

                                    <td>
                                      {`INR ${formatTwoDecimals(item.tax_addition_in_inr)}`}
                                    </td>

                                    <td>
                                      {`INR ${formatTwoDecimals(item.tax_deduction_in_inr)}`}
                                    </td>

                                    <td>
                                      {`INR ${formatTwoDecimals(item.total_charges_in_inr)}`}
                                    </td>

                                    <td>
                                      {`INR ${formatTwoDecimals(item.total_base_cost_in_inr)}`}
                                    </td>

                                    <td>
                                      {`INR ${formatTwoDecimals(item.all_inclusive_cost_in_inr)}`}
                                    </td>
                          ...
                          <table className="w-100">
                            ...
                            <tbody>
                              ...
                              <tr>
                                ...
                                <td className="fw-bold">{formatTwoDecimals(taxSummary.total_inclusive_cost_in_inr ?? taxSummary.total_inclusive_cost ?? 0)}</td>
                              </tr>
                            </tbody>
                          </table>
                        </div>
                        ...
                        <div className="tbl-container me-2 mt-3">
                          ...
                          <thead>
                                  <tr>
                                    ...
                                    <th style={{ width: "150px" }}>
                                      Service Certificate Advance Amount (INR)
                                    </th>
                                  </tr>
                                  ...
                                  <tr key={idx}>
                                    ...
                                    <td style={{ width: "150px" }}>
                                      {(() => {
                                        ...
                                        return `INR ${formatTwoDecimals(inrAmount)}`;
                                      })()}
                                    </td>
                                  </tr>
                                ))
             ...
             <Modal.Body>
               <div className="container-fluid p-0">
                 ...
                 <div className="row mb-3">
                   <div className="col-md-6">
                     <div className="mb-3">
                      <label className="form-label fw-bold">Conversion Rate</label>
                             ...
                             .map((item, rowIndex) => (
                              ...
                              />
                                ...
                                type="text"
                                       className="form-control"
                                value={
                                  typeof item.taxChargePerUom === "string" &&
                                ...
                                disabled
                                 ...
                                 <td>
                                   <input
                                     ...
                                     className="form-control"
                                value={(() => {
                                  const inr = parseFloat(item.amount_inr || 0);
...
export default PodDomesticDetail;
