...
const LabourRateDetails = () => {
    ...
    const fetchRateDetails = async (page) => {
        ...
        try {
            const response = await axios.get(
                `${baseURL}labour_rate_details/${id}.json?page=${page}&per_page=10&token=${token}`
            );
            setRateDetails(response.data);
            console.log("rate details:", response.data)
            // setLoading(false);
    ...
    const handleSubmit = async () => {
        ...
        try {
            const response = await axios.patch(
                `${baseURL}labour_rate_details/${id}/update_status.json?token=${token}`,
    ...
    const handleApplyFilters = () => {
        ...
        axios
            .get(
                `${baseURL}labour_rate_details/${id}.json?search=${search}&token=${token}`
                ...
                <div className="module-data-section p-4">
                    ...
                    {/* {status && status.toLowerCase() === "draft" && (
                        <div className="d-flex justify-content-end m-2">
                            ...
                            <Link
                                to={`/labour-rate-edit/${id}?token=${token}`}
