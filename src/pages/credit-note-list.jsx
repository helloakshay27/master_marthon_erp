...
const CreditNoteList = () => {
  ...
  const handleSubmit = () => {
    ...
    axios
      .patch(`${baseURL}credit_notes/update_bulk_status?token=${token}`, data)
      .then((response) => {
  ...
  useEffect(() => {
    if (fromStatus) {
      ...
      axios
        .get(
          `${baseURL}credit_notes?page=1&token=${token}&q[status_eq]=${fromStatus}`
        )
        .then((response) => {
          const transformedData = response.data.credit_notes.map(
            (entry, index) => {
                ...
                <div className="col-md-6">
                  <div className="d-flex justify-content-end align-items-center gap-3">
                    ...
                    <button
                      ...
                      onClick={() =>
                        navigate(`/credit-note-create?token=${token}`)
                      }
                    >
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
