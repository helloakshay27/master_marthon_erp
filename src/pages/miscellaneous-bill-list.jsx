...
const MiscellaneousBillList = () => {
  ...
  useEffect(() => {
    if (fromStatus) {
      ...
      axios
        .get(
          `${baseURL}miscellaneous_bills?page=1&token=${token}&q[status_eq]=${fromStatus}`
        )
        .then((response) => {
          const transformedData = response.data.bills.map((entry, index) => {
            let status = entry.status;
  ...
  const fetchFilteredData2 = (status) => {
    const url = `${baseURL}miscellaneous_bills?page=1&token=${token}${
    ...
    { field: "site_name", headerName: "Sub Project", width: 150 },
    {
      ...
      renderCell: (params) =>
        params.value && params.row.id ? (
          <Link
            to={`/miscellaneous-bill-details/${params.row.id}?token=${token}`}
          >
